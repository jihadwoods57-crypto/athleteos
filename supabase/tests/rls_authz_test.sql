...
-- (0211: acks are windowed, so open today's instance around `now()` first. The suite runs at any
-- hour and a 4:45 AM roll call is closed at most of them.)
select _superuser();
update commitment_instances set starts_at = now() - interval '1 minute', respond_by_at = now() + interval '5 minutes'
...
-- ================================================================ wake-up roll call (0211)
-- The promises the Wake-Up Roll Call makes: the verdict is a pure function of the timestamps,
-- a tap outside the window is refused (before it opens AND after it closes), a late tap is
-- recorded as late and never upgraded, the coach's new reads and writes stop at their own book,
-- an edit re-times only the future, and the service-role materializer is unreachable from a
-- client. Reuses the 0138 cast: team T1, coach_1 (staff), coach_2 (stranger), athletes e1/e2 and
-- commitment ccccdddd-...-c1 with today's instance.
select _superuser();
select set_config('request.jwt.claim.sub', '', false);
update public.feature_flags set kill_switch = false where name = 'verified_commitments';
...
select _ok(rollcall_late_min(now() - interval '4 minutes', now() - interval '5 minutes') = 1
       ...
       and rollcall_late_min(now() - interval '6 minutes', now() - interval '5 minutes') is null,
  '0211: minutes late round up and are null when not late');

-- ---- the windows, pure ----
select _ok(commitment_opens_at('2026-09-01T10:00:00Z'::timestamptz, '2026-09-01T10:05:00Z'::timestamptz, 360::smallint, null::smallint) = '2026-09-01T09:05:00Z'::timestamptz,
...
select _ok(rollcall_closes_at('practice', '2026-09-01T10:05:00Z'::timestamptz, '2026-09-01T10:00:00Z'::timestamptz, null::timestamptz) is null,
  '0211: every other type keeps its pre-0211 behaviour: no ends_at, no close');

-- ---- the ack window on the athlete path ----
-- e2 starts the section unanswered. Today's instance is re-timed by hand (superuser) to walk it
...
-- not open yet: starts in 2h, deadline in 2h05, so it opens in 65 minutes
update commitment_instances set starts_at = now() + interval '2 hours', respond_by_at = now() + interval '125 minutes', ends_at = null, status = 'scheduled'
 where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date;
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok(_try($f$ select ack_commitment((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)) $f$) like 'denied(P0001)%not open yet%',
  '0211: an athlete cannot check in before the roll call opens (no midnight answers to a 6 AM roll call)');

-- closed: started 3h ago, deadline 2h55m ago, closed 55m ago
select _superuser();
update commitment_instances set starts_at = now() - interval '3 hours', respond_by_at = now() - interval '175 minutes'
 where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date;
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok(_try($f$ select ack_commitment((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)) $f$) like 'denied(P0001)%closed%',
  '0211: an athlete cannot check in after the roll call closes');
select _superuser();
select _ok((select acknowledged_at is null from commitment_responses
             where athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
               and instance_id = (select id from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)),
  '0211: a refused tap writes nothing');

-- late window: started 10m ago, deadline 5m ago, closes in 1h55
update commitment_instances set starts_at = now() - interval '10 minutes', respond_by_at = now() - interval '5 minutes'
 where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date;
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok(_try($f$ select ack_commitment((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)) $f$) = 'ok',
...
select _ok((select r.ack_source = 'app'
             ...
             where r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
               and i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.occurs_on = current_date),
  '0211: the late tap is recorded as late, about five minutes, from the app');
-- the first tap stands: a second tap (even after the close) returns the same stamp
create temp table _rc_stamp as
  select acknowledged_at as at from commitment_responses
   where athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
     and instance_id = (select id from commitment_instances
                         where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date);
update commitment_instances set starts_at = now() - interval '3 hours', respond_by_at = now() - interval '175 minutes'
...
select _ok(_try($f$ select ack_commitment((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)) $f$) = 'ok',
  '0211: a second tap on an answered roll call is idempotent even once it has closed');
select _superuser();
select _ok((select r.acknowledged_at = s.at from commitment_responses r join _rc_stamp s on true
             where r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
               and r.instance_id = (select id from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)),
  '0211: the first stamp survives the second tap');
drop table _rc_stamp;

-- ---- the ack window on the lock-screen path (service role, athlete named by the code) ----
update commitment_responses set status = 'pending', acknowledged_at = null, ack_source = null, updated_at = now()
 where athlete_id = 'eeee0000-0000-0000-0000-0000000000e1'
   and instance_id = (select id from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date);
select _ok(_try($f$ select ack_commitment_by_token((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
             'eeee0000-0000-0000-0000-0000000000e1') $f$) like 'denied(P0001)%closed%',
  '0211: the lock-screen path refuses a closed roll call too');
...
select _ok((select r.ack_source = 'lockscreen'
             ...
             where r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e1'
               and i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.occurs_on = current_date),
  '0211: a lock-screen tap inside the grace is on standard and says where it came from');
-- The re-timing above put today's deadline back in the future, which would silently turn e2's
-- late answer into an on-time one for every read below. Pin e2 five minutes past it.
update commitment_responses r set acknowledged_at = i.respond_by_at + interval '5 minutes'
  from commitment_instances i
 where i.id = r.instance_id and r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
   and i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.occurs_on = current_date;

-- ---- the reads carry the verdict ----
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok((
  with e as (select jsonb_array_elements(my_commitments(current_date, current_date)) o)
  select bool_and(jsonb_exists(e.o, k)) from e, unnest(array[
    'verdict','late_min','closes_at','opens_at','grace_min','ack_source','last_nudge_at','coach_name'
  ]) k),
  '0211 contract: my_commitments carries the wake-up keys');
select _ok((select o->>'verdict' = 'late' and (o->>'late_min')::int >= 5 and o->>'ack_source' = 'app'
              from jsonb_array_elements(my_commitments(current_date, current_date)) o
             where o->>'instance_id' = (select id::text from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)),
  '0211 contract: the athlete reads their own late verdict');
select _as('11111111-0000-0000-0000-000000000001');
select _ok((
  with e as (select jsonb_array_elements(
      commitment_board('77777777-1111-0000-0000-000000000001', null, current_date)) o limit 1)
  select bool_and(jsonb_exists(e.o, k)) from e, unnest(array[
    'closes_at','opens_at','grace_min','last_nudge_at','coach_name','standing_message','message_override','occurs_on'
  ]) k),
  '0211 contract: commitment_board carries the wake-up instance keys');
select _ok((
  with e as (select jsonb_array_elements(
      commitment_board('77777777-1111-0000-0000-000000000001', null, current_date)) o limit 1),
       r as (select jsonb_array_elements(e.o -> 'rows') ro from e)
  select bool_and(jsonb_exists(r.ro, 'verdict') and jsonb_exists(r.ro, 'late_min') and jsonb_exists(r.ro, 'ack_source')) from r),
  '0211 contract: each board row carries verdict, late_min and ack_source');
select _ok((
  with e as (select jsonb_array_elements(
      commitment_board('77777777-1111-0000-0000-000000000001', null, current_date)) o),
       r as (select jsonb_array_elements(e.o -> 'rows') ro from e
              where e.o->>'commitment_id' = 'ccccdddd-0000-0000-0000-0000000000c1')
  select bool_and(case ro->>'athlete_id'
                    when 'eeee0000-0000-0000-0000-0000000000e2' then ro->>'verdict' = 'late' and (ro->>'late_min')::int = 5
                    when 'eeee0000-0000-0000-0000-0000000000e1' then ro->>'verdict' = 'on_standard' and ro->>'ack_source' = 'lockscreen'
                    else true end)
    from r),
  '0211: the coach board splits e2 late and e1 on standard');

-- ---- accountability counts the split without moving the arithmetic ----
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok((select (a->>'wake_late')::int >= 1
              and (a->>'wake_done')::int = (a->>'wake_on_standard')::int + (a->>'wake_late')::int
              and (a->>'earned')::int % 10 = 0
             from athlete_accountability('eeee0000-0000-0000-0000-0000000000e2', current_date, current_date) a),
  '0211: accountability exposes wake_late and keeps the 10/30/60 arithmetic');

-- ---- coach summary: staff of the book only ----
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select rollcall_summary('ccccdddd-0000-0000-0000-0000000000c1', 14) $f$) = 'ok',
  '0211: staff can read the roll-call summary');
select _ok((select bool_and(jsonb_exists(o, 'on_standard') and jsonb_exists(o, 'late') and jsonb_exists(o, 'missed')
                            and jsonb_exists(o, 'pending') and jsonb_exists(o, 'total') and jsonb_exists(o, 'occurs_on'))
              from jsonb_array_elements(rollcall_summary('ccccdddd-0000-0000-0000-0000000000c1', 14)) o),
  '0211 contract: the summary carries the verdict counts per occurrence');
select _as('22222222-0000-0000-0000-000000000002');
select _ok(_try($f$ select rollcall_summary('ccccdddd-0000-0000-0000-0000000000c1', 14) $f$) <> 'ok',
  '0211: a stranger coach cannot read another team''s summary');
select _as('eeee0000-0000-0000-0000-0000000000e1');
select _ok(_try($f$ select rollcall_summary('ccccdddd-0000-0000-0000-0000000000c1', 14) $f$) <> 'ok',
  '0211: an athlete cannot read the team summary');

-- ---- today's message: staff writes the occurrence, athletes read it, strangers cannot ----
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select set_instance_message((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date), 'Scout meet at 7. Bring the sheet.') $f$) = 'ok',
  '0211: staff can set today''s message');
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok((select o->>'message' = 'Scout meet at 7. Bring the sheet.'
              from jsonb_array_elements(my_commitments(current_date, current_date)) o
             where o->>'instance_id' = (select id::text from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date)),
  '0211: the athlete reads today''s message over the standing one');
select _ok(_try($f$ select set_instance_message((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date), 'nope') $f$) <> 'ok',
  '0211: an athlete cannot write today''s message');
select _as('22222222-0000-0000-0000-000000000002');
select _ok(_try($f$ select set_instance_message((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date), 'nope') $f$) <> 'ok',
  '0211: a stranger coach cannot write another team''s message');
select _superuser();
select _ok((select message_override = 'Scout meet at 7. Bring the sheet.' from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
  '0211: the refused writes left the coach''s message intact');
select _ok((select char_length(message) = 1000 from (
  select (regexp_replace(repeat('x', 1000), '', '')) as message) t),
  '0211: (sanity) a 1000-char message is the new ceiling');
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try(format($f$ select set_instance_message((select id from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date), %L) $f$, repeat('y', 1000))) = 'ok',
  '0211: a 1000-character morning message is accepted');
select _superuser();

-- ---- per-athlete ping: its own cooldown, never the roster''s ----
-- e2 answered late above; reset e2 unanswered for this block, keep e1 answered.
update commitment_responses set status = 'pending', acknowledged_at = null, ack_source = null, last_nudge_at = null, updated_at = now()
 where athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
   and instance_id = (select id from commitment_instances
                       where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date);
update commitment_instances set last_nudge_at = null
 where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date;
select _ok((select (j->>'ok')::boolean and j->'athlete_ids' = jsonb_build_array('eeee0000-0000-0000-0000-0000000000e2')
             from rollcall_nudge_claim(
               (select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
               '11111111-0000-0000-0000-000000000001', 10, 'eeee0000-0000-0000-0000-0000000000e2') j),
  '0211: a single ping targets exactly that athlete');
select _ok((select j->>'ok' = 'false' and j->>'reason' = 'rate_limited'
             from rollcall_nudge_claim(
               (select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
               '11111111-0000-0000-0000-000000000001', 10, 'eeee0000-0000-0000-0000-0000000000e2') j),
  '0211: pinging the same athlete again inside the cooldown is refused');
select _ok((select last_nudge_at is null from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
  '0211: a single ping does not spend the roster-wide cooldown');
select _ok((select (j->>'ok')::boolean and j->'athlete_ids' = '[]'::jsonb
             from rollcall_nudge_claim(
               (select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
               '11111111-0000-0000-0000-000000000001', 10, 'eeee0000-0000-0000-0000-0000000000e1') j),
  '0211: pinging an athlete who already answered reaches nobody, and is not an error');
select _ok((select j->>'reason' = 'not_authorized'
             from rollcall_nudge_claim(
               (select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
               '22222222-0000-0000-0000-000000000002', 10, 'eeee0000-0000-0000-0000-0000000000e2') j),
  '0211: a stranger coach cannot ping an athlete on another team');
select _ok((select (j->>'ok')::boolean and jsonb_exists(j, 'closes_at')
             from rollcall_nudge_claim(
               (select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
               '11111111-0000-0000-0000-000000000001', 10) j),
  '0211: the roster-wide nudge still works and now hands back the close for the code');

-- ---- the service-role surface is unreachable from a client ----
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select materialize_active_commitments() $f$) <> 'ok',
  '0211: a coach cannot execute materialize_active_commitments');
select _ok(_try($f$ select materialize_commitment('ccccdddd-0000-0000-0000-0000000000c1', current_date, current_date) $f$) <> 'ok',
  '0211: a coach cannot execute materialize_commitment directly');
select _ok(_try($f$ select resync_commitment_instances('ccccdddd-0000-0000-0000-0000000000c1') $f$) <> 'ok',
  '0211: a coach cannot execute resync_commitment_instances directly');
select _as('eeee0000-0000-0000-0000-0000000000e2');
select _ok(_try($f$ select materialize_active_commitments() $f$) <> 'ok',
  '0211: an athlete cannot execute materialize_active_commitments');
select _superuser();
select _ok(_try($f$ select materialize_active_commitments() $f$) = 'ok',
  '0211: the service role materializes');
select _ok(exists (select 1 from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1'
...
select materialize_commitment('ccccdddd-0000-0000-0000-0000000000c1', current_date + 1, current_date + 2);

-- ---- an edit re-times the future and leaves history alone ----
-- Today's instance was hand-timed above (started 2 minutes ago). Move the standing rule to
-- 5:00 AM with a 20-minute grace: every not-yet-started occurrence follows, today does not.
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1',
    'team_id','77777777-1111-0000-0000-000000000001',
    'type','morning_roll_call','title','Wake-Up Roll Call','message','Everyone up?',
    'audience_kind','team',
    'repeat_days', jsonb_build_array(0,1,2,3,4,5,6),
    'starts_min', 300, 'respond_by_min', 320, 'opens_min', 290)) $f$) = 'ok',
  '0211: the coach re-times the roll call');
select _superuser();
select _ok((select bool_and(
              extract(hour from i.starts_at at time zone c.timezone) * 60 + extract(minute from i.starts_at at time zone c.timezone) = 300
              and extract(hour from i.respond_by_at at time zone c.timezone) * 60 + extract(minute from i.respond_by_at at time zone c.timezone) = 320)
             from commitment_instances i join commitments c on c.id = i.commitment_id
            where i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.starts_at > now() and i.status = 'scheduled'),
  '0211: every future occurrence now starts at 5:00 with a 5:20 deadline, in the team''s zone');
select _ok((select starts_at between now() - interval '3 minutes' and now() from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = current_date),
  '0211: today''s already-started occurrence is untouched by the edit');
-- drop the weekday of the next future occurrence: it is cancelled, not deleted
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try(format($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1',
    'team_id','77777777-1111-0000-0000-000000000001',
    'type','morning_roll_call','title','Wake-Up Roll Call','message','Everyone up?',
    'audience_kind','team',
    'repeat_days', (select jsonb_agg(d) from generate_series(0,6) d where d <> %s),
    'starts_min', 300, 'respond_by_min', 320, 'opens_min', 290)) $f$,
    (select extract(dow from occurs_on)::int from commitment_instances
      where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and starts_at > now() and status = 'scheduled'
      order by occurs_on limit 1))) = 'ok',
  '0211: the coach removes a day');
select _superuser();
select _ok((select count(*) >= 1 from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and starts_at > now() and status = 'cancelled'),
  '0211: a removed day''s future occurrence is cancelled, never deleted');
select _ok((select count(*) from commitment_responses r join commitment_instances i on i.id = r.instance_id
             where i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.occurs_on = current_date) >= 2,
  '0211: today''s responses are still there');
-- add it back: scheduled again
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1',
    'team_id','77777777-1111-0000-0000-000000000001',
    ...
    'repeat_days', jsonb_build_array(0,1,2,3,4,5,6),
    'starts_min', 300, 'respond_by_min', 320, 'opens_min', 290)) $f$) = 'ok',
  '0211: the coach adds the day back');
select _superuser();
select _ok((select count(*) = 0 from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and starts_at > now() and status = 'cancelled'),
  '0211: the occurrence is scheduled again');
-- pause: the future is cancelled; resume: it is back
select _as('11111111-0000-0000-0000-000000000001');
select _try($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1','team_id','77777777-1111-0000-0000-000000000001',
    'type','morning_roll_call','title','Wake-Up Roll Call','message','Everyone up?','audience_kind','team',
    'repeat_days', jsonb_build_array(0,1,2,3,4,5,6),'starts_min', 300,'respond_by_min', 320,'opens_min', 290,
    'active', false)) $f$);
select _superuser();
select _ok((select count(*) = 0 from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and starts_at > now() and status = 'scheduled'),
  '0211: pausing cancels every future occurrence, so no push can fire');
select _as('11111111-0000-0000-0000-000000000001');
select _try($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1','team_id','77777777-1111-0000-0000-000000000001',
    'type','morning_roll_call','title','Wake-Up Roll Call','message','Everyone up?','audience_kind','team',
...
select _ok((select count(*) >= 1 from commitment_instances
             where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and starts_at > now() and status = 'scheduled'),
  '0211: resuming brings the future back');

-- ---- the claims hand back what the push needs ----
select _ok(_try($f$ select type, message, coach_name, starts_at, closes_at, fires_at, timezone
                    from claim_due_commitment_reminders(10, 1) $f$) = 'ok',
  '0211 contract: claim_due_commitment_reminders returns the coach-voice columns');
select _ok(_try($f$ select type, action_label, respond_by_at, closes_at
                    from claim_missed_commitments(10, null, 1) $f$) = 'ok',
  '0211 contract: claim_missed_commitments returns the late-push columns');
select _as('eeee0000-0000-0000-0000-0000000000e2');
...
select _ok(_try($f$ select rollcall_upcoming('ccccdddd-0000-0000-0000-0000000000c1', 7) $f$) = 'ok',
  '0215: staff can read the week ahead');
select _ok((select jsonb_array_length(rollcall_upcoming('ccccdddd-0000-0000-0000-0000000000c1', 7)) >= 2),
  '0215: the week ahead materializes future occurrences on the way in');
select _as('eeee0000-0000-0000-0000-0000000000e1');
select _ok(_try($f$ select rollcall_upcoming('ccccdddd-0000-0000-0000-0000000000c1', 7) $f$) <> 'ok',
  '0215: an athlete cannot read the coach''s week ahead');
select _as('22222222-0000-0000-0000-000000000002');
select _ok(_try($f$ select rollcall_upcoming('ccccdddd-0000-0000-0000-0000000000c1', 7) $f$) <> 'ok',
  '0215: a stranger coach cannot read another team''s week ahead');
...
select _ok((select count(*) from _rc_next) = 1, '0215: (fixture) a future occurrence exists');
...
select _ok(_try($f$ select set_instance_schedule((select id from _rc_next), 390::smallint, false, null, 'Late game') $f$) = 'ok',
  '0215: staff can move one day''s wake-up time');
select _superuser();
select _ok((select starts_override_min = 390 and note = 'Late game' and schedule_set_by = '11111111-0000-0000-0000-000000000001'
  ...
  '0215: the override, the note and who set it are recorded on the occurrence');
select _ok((select _rc_min_of(i.starts_at, c.timezone) = 390
  ...
  '0215: the day is re-timed keeping the rule''s grace delta');
select _ok((select starts_min from commitments where id = 'ccccdddd-0000-0000-0000-0000000000c1') <> 390,
  '0215: the standing rule''s time is untouched by a per-day move');
...
select _ok((select (o->>'starts_min')::int = 390 and (o->>'rule_starts_min')::int <> 390
  ...
  '0215: the board reports the day''s effective minute and the rule''s minute');
select _as('eeee0000-0000-0000-0000-0000000000e1');
select _ok((select (o->>'starts_min')::int = 390
             ...
             where o->>'instance_id' = (select id::text from _rc_next)),
  '0215: the athlete''s card reads the moved time');
...
select _ok(_try($f$ select set_instance_schedule((select id from _rc_next), 400::smallint) $f$) <> 'ok',
  '0215: an athlete cannot move a roll call');
select _as('22222222-0000-0000-0000-000000000002');
select _ok(_try($f$ select set_instance_schedule((select id from _rc_next), 400::smallint) $f$) <> 'ok',
  '0215: a stranger coach cannot move another team''s roll call');
select _superuser();
select _ok((select starts_override_min = 390 from commitment_instances where id = (select id from _rc_next)),
  '0215: the refused writes left the coach''s move intact');
...
select _ok(_try($f$ select set_instance_schedule((select id from _rc_next), null, false, true, 'Travel day') $f$) = 'ok',
  '0215: staff can skip one day');
select _superuser();
select _ok((select skipped and status = 'cancelled' from commitment_instances where id = (select id from _rc_next)),
  '0215: a skipped day is a cancelled occurrence with the flag set');
select _as('eeee0000-0000-0000-0000-0000000000e1');
select _ok((select o->>'instance_status' = 'cancelled'
              from jsonb_array_elements(my_commitments((select occurs_on from _rc_next), (select occurs_on from _rc_next))) o
             where o->>'instance_id' = (select id::text from _rc_next)),
  '0215: the athlete read carries the cancelled status the client hides');
select _as('11111111-0000-0000-0000-000000000001');
select _ok(_try($f$ select upsert_commitment(jsonb_build_object(
    'id','ccccdddd-0000-0000-0000-0000000000c1',
    'team_id','77777777-1111-0000-0000-000000000001',
...
select _ok(_try($f$ select set_instance_schedule((select id from commitment_instances
  ...
  '0215: an occurrence that has started cannot be re-timed');
select _superuser();
drop table _rc_next;
...
insert into _v3_calls values
  ...
  ('select materialize_rollcall_ahead_svc(''ccccdddd-0000-0000-0000-0000000000c1''::uuid, 14)'),
  ('select rollcall_notify_claim(''ccccdddd-0000-0000-0000-0000000000c1''::uuid, ''11111111-0000-0000-0000-000000000001''::uuid, 60)'),
...
select _ok((select pg_get_functiondef('claim_rollcall_notices(uuid,int)'::regprocedure) ilike '%for update of r skip locked%'),
  '0247: the claim skips rows another tick holds (FOR UPDATE SKIP LOCKED), never waits and double-claims');
select _ok((select bool_and(claimed_at is not null) from _v3_c1), '0247: every claimed row carries its claim time');
-- a settle or release whose claim time is not the row's current one belongs to a claim that lapsed
select _ok(settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at - interval '3 minutes', 'sent', true)) from _v3_c1), true) = 0,
  '0247: settle does nothing for a claim it no longer owns');
select _ok(settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'claimed_at', claimed_at - interval '3 minutes')) from _v3_c1), false) = 0,
  '0247: nor does a release');
select _ok(settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id)) from _v3_c1), false) = 0,
  '0247: a release with no claim time never clears a claim in flight');
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', athlete_id = 'eeee0000-0000-0000-0000-0000000000e1')) from _v3_c1), true);
...
select _ok((select notified_at is null and notice_settled_at is not null and notice_claimed_at is null
              from commitment_responses where instance_id = (select id from _v3_i) and athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'),
  '0247: a row settled with no push sent (no token, silent) is settled but never stamped told');
...
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', athlete_id = 'eeee0000-0000-0000-0000-0000000000e1')) from _v3_c2), true);
...
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', athlete_id = 'eeee0000-0000-0000-0000-0000000000e1')) from _v3_c3), true);
select _ok(not exists (select 1 from claim_rollcall_notices('ccccdddd-0000-0000-0000-0000000000c1', 500) x where x.instance_id = (select id from _v3_i)),
...
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', athlete_id = 'eeee0000-0000-0000-0000-0000000000e1')) from _v3_c4), true);
...
select _ok((select (r->>'notified_at') is null
              from jsonb_array_elements(rollcall_arming((select id from _v3_i))->'rows') r
             where r->>'athlete_id' = 'eeee0000-0000-0000-0000-0000000000e2'),
  '0247: an athlete no push reached still reads "Hasn''t been told"');
...
select set_wake_alarm_armed_svc((select id from _v3_i), 'eeee0000-0000-0000-0000-0000000000e2', true);
select _ok((rollcall_arm_remind_claim((select id from _v3_i), '11111111-0000-0000-0000-000000000001', 10)->>'reason') = 'nobody_to_remind'
       and (select arm_reminded_at is null from commitments where id = 'ccccdddd-0000-0000-0000-0000000000c1'),
  '0247: with every alarm set there is nobody to remind, and the cooldown is not spent');
select set_wake_alarm_armed_svc((select id from _v3_i), 'eeee0000-0000-0000-0000-0000000000e2', false);
...
select _ok(exists (select 1 from rollcall_remind_rows_svc('ccccdddd-0000-0000-0000-0000000000c1', array['eeee0000-0000-0000-0000-0000000000e2']::uuid[])),
  '0247: (control) the remind rows name e2''s unarmed mornings');
update commitments set arm_reminded_at = null where id = 'ccccdddd-0000-0000-0000-0000000000c1';
update commitments set active = false where id = 'ccccdddd-0000-0000-0000-0000000000c1';
select _ok((rollcall_arm_remind_claim((select id from _v3_i), '11111111-0000-0000-0000-000000000001', 10)->>'reason') = 'no_instance'
       and not exists (select 1 from rollcall_remind_rows_svc('ccccdddd-0000-0000-0000-0000000000c1', array['eeee0000-0000-0000-0000-0000000000e2']::uuid[]))
       and (select arm_reminded_at is null from commitments where id = 'ccccdddd-0000-0000-0000-0000000000c1'),
  '0247: a paused roll call reminds nobody and spends no cooldown');
update commitments set active = true where id = 'ccccdddd-0000-0000-0000-0000000000c1';
update public.feature_flags set kill_switch = true where name = 'verified_commitments';
select _ok((rollcall_arm_remind_claim((select id from _v3_i), '11111111-0000-0000-0000-000000000001', 10)->>'reason') = 'disabled'
       and not exists (select 1 from rollcall_remind_rows_svc('ccccdddd-0000-0000-0000-0000000000c1', array['eeee0000-0000-0000-0000-0000000000e2']::uuid[]))
       and not exists (select 1 from claim_rollcall_notices('ccccdddd-0000-0000-0000-0000000000c1', 500))
       and (select arm_reminded_at is null from commitments where id = 'ccccdddd-0000-0000-0000-0000000000c1'),
  '0247: with the kill switch thrown nothing is claimed or reminded');
update public.feature_flags set kill_switch = false where name = 'verified_commitments';

-- ---- Task 4 review: a remind never clobbers a pending notice ----
-- Start from nothing pending: whatever the claim holds for this roll call is settled (told for e1).
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', athlete_id = 'eeee0000-0000-0000-0000-0000000000e1'))
                                  from claim_rollcall_notices('ccccdddd-0000-0000-0000-0000000000c1', 500)), true);
select _ok(not exists (select 1 from claim_rollcall_notices('ccccdddd-0000-0000-0000-0000000000c1', 500) x where x.instance_id = (select id from _v3_i)),
  '0247 T4: (fixture) nothing pending on the section''s morning');
create temp table _v3_before as
  select athlete_id, notified_starts_at, notice_settled_at, notified_at from commitment_responses where instance_id = (select id from _v3_i);
-- The suite is one transaction (now() does not move), so e1's told stamp on this morning is cleared
-- first: the remind must be what puts it back.
update commitment_responses set notified_at = null
 where instance_id = (select id from _v3_i) and athlete_id = 'eeee0000-0000-0000-0000-0000000000e1';
-- The coach moves the morning, then presses Remind before the cron runs.
update commitment_instances set starts_at = starts_at + interval '10 minutes' where id = (select id from _v3_i);
create temp table _v3_rr as
  select * from rollcall_remind_rows_svc('ccccdddd-0000-0000-0000-0000000000c1', array['eeee0000-0000-0000-0000-0000000000e1']::uuid[]) x
   where x.instance_id = (select id from _v3_i);
select _ok((select count(*) from _v3_rr) = 1 and (select kind from _v3_rr) = 'remind' and (select claimed_at from _v3_rr) is null,
  '0247 T4: (fixture) the moved morning is in e1''s remind rows, with no claim');
select _ok(settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', true, 'remind', true)) from _v3_rr), true) = 1,
  '0247 T4: a delivered remind settles its row');
select _ok((select r.notified_starts_at = b.notified_starts_at and r.notice_settled_at = b.notice_settled_at
                   and r.notified_at is not null and r.notice_claimed_at is null
              from commitment_responses r join _v3_before b on b.athlete_id = r.athlete_id
             where r.instance_id = (select id from _v3_i) and r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e1'),
  '0247 T4: a remind stamps notified_at only; the claim''s bookkeeping (told time, settled) is untouched');
select _ok(settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', false, 'remind', true)) from _v3_rr), true) = 0,
  '0247 T4: an undelivered remind touches nothing');
create temp table _v3_c5 as select * from claim_rollcall_notices('ccccdddd-0000-0000-0000-0000000000c1', 500) x where x.instance_id = (select id from _v3_i);
select _ok((select kind from _v3_c5 where athlete_id = 'eeee0000-0000-0000-0000-0000000000e1') = 'moved',
  '0247 T4: move, then Remind before the cron: the "moved" notice still goes out');
-- Ruling (b): e2 was never actually told about this roll call (every push to them was refused).
select _ok(not exists (select 1 from commitment_responses r join commitment_instances i on i.id = r.instance_id
                        where i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e2'
                          and r.notified_at is not null),
  '0247 T4: (fixture) e2 has never been told about this roll call');
select _ok((select kind from _v3_c5 where athlete_id = 'eeee0000-0000-0000-0000-0000000000e2') = 'assigned',
  '0247 T4: a change for an athlete never actually told is said as "assigned", not "moved"');
select settle_rollcall_notices((select jsonb_agg(jsonb_build_object('response_id', response_id, 'starts_at', starts_at, 'off', off, 'claimed_at', claimed_at, 'sent', false)) from _v3_c5), true);

-- ---- Task 4 review: the coach's notify, one roll call at a time ----
select _ok((rollcall_notify_claim('ccccdddd-0000-0000-0000-0000000000c1', '11111111-0000-0000-0000-000000000001', 60)->>'cooldown') = 'false',
  '0247 T4: the owning coach''s notify runs');
select _ok((select (j->>'ok')::boolean and (j->>'cooldown')::boolean
              from (select rollcall_notify_claim('ccccdddd-0000-0000-0000-0000000000c1', '11111111-0000-0000-0000-000000000001', 60) j) x),
  '0247 T4: a second notify inside the cooldown is ok with cooldown, nothing re-run');
select _ok((rollcall_notify_claim('ccccdddd-0000-0000-0000-0000000000c1', '11111111-0000-0000-0000-000000000002', 60)->>'reason') = 'not_authorized',
  '0247 T4: a stranger coach cannot notify');
update commitments set notify_claimed_at = null where id = 'ccccdddd-0000-0000-0000-0000000000c1';
update public.feature_flags set kill_switch = true where name = 'verified_commitments';
select _ok((rollcall_notify_claim('ccccdddd-0000-0000-0000-0000000000c1', '11111111-0000-0000-0000-000000000001', 60)->>'reason') = 'disabled'
       and materialize_rollcall_ahead_svc('ccccdddd-0000-0000-0000-0000000000c1', 14) = 0
       and (select notify_claimed_at is null from commitments where id = 'ccccdddd-0000-0000-0000-0000000000c1'),
  '0247 T4: with the kill switch thrown notify refuses and materializes nothing');
update public.feature_flags set kill_switch = false where name = 'verified_commitments';
update commitments set active = false where id = 'ccccdddd-0000-0000-0000-0000000000c1';
select _ok((rollcall_notify_claim('ccccdddd-0000-0000-0000-0000000000c1', '11111111-0000-0000-0000-000000000001', 60)->>'reason') = 'no_instance'
       and materialize_rollcall_ahead_svc('ccccdddd-0000-0000-0000-0000000000c1', 14) = 0,
  '0247 T4: a paused roll call is neither notified nor materialized');
update commitments set active = true where id = 'ccccdddd-0000-0000-0000-0000000000c1';
-- Earlier sections edit c1; pin the fixture: every day, open-ended.
update commitments set repeat_days = array[0,1,2,3,4,5,6], ends_on = null, starts_on = least(starts_on, current_date)
 where id = 'ccccdddd-0000-0000-0000-0000000000c1';
create temp table _v3_d5 as
  select (now() at time zone c.timezone)::date + 5 as d from commitments c where c.id = 'ccccdddd-0000-0000-0000-0000000000c1';
delete from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = (select d from _v3_d5);
select _ok(materialize_rollcall_ahead_svc('ccccdddd-0000-0000-0000-0000000000c1', 14) >= 1,
  '0247 T4: notify''s materialize creates the missing morning');
select _ok(exists (select 1 from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = (select d from _v3_d5)),
  '0247 T4: notify''s materialize fills this roll call''s next 14 days');

-- ---- Task 4 review: remind is cooled down per roll call, not per morning ----
update commitments set arm_reminded_at = null where id = 'ccccdddd-0000-0000-0000-0000000000c1';
update commitment_responses r set alarm_armed_at = null, status = 'pending', acknowledged_at = null
  from commitment_instances i
 where i.id = r.instance_id and i.commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and i.occurs_on = (select d from _v3_d5)
   and r.athlete_id = 'eeee0000-0000-0000-0000-0000000000e2';
select _ok((rollcall_arm_remind_claim((select id from _v3_i), '11111111-0000-0000-0000-000000000001', 10)->>'ok') = 'true',
  '0247 T4: (fixture) a remind on one morning');
select _ok((rollcall_arm_remind_claim((select id from commitment_instances where commitment_id = 'ccccdddd-0000-0000-0000-0000000000c1' and occurs_on = (select d from _v3_d5)),
                                      '11111111-0000-0000-0000-000000000001', 10)->>'reason') = 'rate_limited',
  '0247 T4: a remind on ANOTHER morning of the same roll call inside the cooldown is refused');

-- ---- Task 4 review: "Notifications off" includes the athlete's own master switch ----
insert into device_tokens (user_id, token, platform) values ('eeee0000-0000-0000-0000-0000000000e1', 'ExponentPushToken[v3-e1]', 'ios')
  on conflict do nothing;
update profiles set notifications_opt_out = false where id = 'eeee0000-0000-0000-0000-0000000000e1';
select _as('11111111-0000-0000-0000-000000000001');
select _ok((select (r->>'can_push')::boolean from jsonb_array_elements(rollcall_arming((select id from _v3_i))->'rows') r
             where r->>'athlete_id' = 'eeee0000-0000-0000-0000-0000000000e1'),
  '0247 T4: (control) an athlete with a phone and notifications on can be pushed');
select _superuser();
update profiles set notifications_opt_out = true where id = 'eeee0000-0000-0000-0000-0000000000e1';
select _as('11111111-0000-0000-0000-000000000001');
select _ok((select not (r->>'can_push')::boolean from jsonb_array_elements(rollcall_arming((select id from _v3_i))->'rows') r
             where r->>'athlete_id' = 'eeee0000-0000-0000-0000-0000000000e1'),
  '0247 T4: an athlete who switched notifications off reads "Notifications off" to the coach');
select _superuser();
update profiles set notifications_opt_out = false where id = 'eeee0000-0000-0000-0000-0000000000e1';
