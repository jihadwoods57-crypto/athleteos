...
import { VC, loadCommitments, loadLocations, loadUpcoming, loadTeamBoard, subscribeTeamBoard, remindMissing } from '../commitment-data.js';
...
import { loadArming, armingFor, remindArm, toldState } from '../rollcall-v3-data.js';

const DAYS = 14;              // the alarm horizon: the server materializes and phones arm this far
...
const clock = (iso) => clockTime(iso).replace(/\s?[AaPp]\.?\s?[Mm]\.?$/, '').trim();

/* ---------------------------------------------------------------- the told line */

function toldHtml(sub) {
  const t = toldState(sub);
  if (!t) return '';
  const nouns = CD.nouns;
  const say = {
    sending: [`Telling your ${nouns} now…`, ''],
    ok: t.sent > 0 ? [`Your ${nouns} were just told.`, 'g'] : ['Saved. Nobody needed a new push.', ''],
  ...
  if (!say[0]) return '';
  return `<p class="rhb-told ${say[1]}" role="status" aria-live="polite">${say[1] === 'g' ? icon('check', 16) : icon(t.state === 'sending' ? 'clock' : 'bell', 16)}<span>${esc(say[0])}</span></p>`;
}

/* ---------------------------------------------------------------- the three bodies */

function noteHtml(key) {
  const n = HUB.note && HUB.note.key === key ? HUB.note : null;
...
function beforeHtml(inst, rule) {
  ...
  return `<section class="rfig rhb-hero" aria-label="${esc(`${c.armed} of ${c.total} alarms set for ${when}`)}">
    ...
    <ul class="card rhb-list" aria-label="Who will ring">${rows.map(stepRow).join('')}</ul>` : ''}
...
function boardBlock(inst, live) {
  ...
  const when = m.mode === 'arrival' || !b.closes_at ? '' : `${m.closed ? 'Closed' : 'Closes'} ${clock(b.closes_at)}`;
  const key = `nudge:${id}`;
  const act = live && !m.closed && m.mode !== 'arrival' && notUp
    ? cooling(key)
  ...
  return `${stale}<div class="rhb-board">${boardHtml(m, { coach: false, when })}</div>
    ${act}${noteHtml(key)}
...
/** The soonest morning that has not started and is not cancelled: what Move and Cancel act on. */
function movable(rows, nowMs = Date.now()) {
  return (rows || []).filter((r) => r && r.instance_id && !r.skipped && r.instance_status !== 'cancelled'
    && Date.parse(r.starts_at || '') > nowMs).sort((a, b) => Date.parse(a.starts_at) - Date.parse(b.starts_at))[0] || null;
}
...
function actionsHtml(rule, rows) {
  ...
  const n = movable(rows);
  const word = n ? dayWord(n, todayIn(rule.timezone)) : '';
...
export default {
  ...
  mount(root, { sub } = {}) {
    ensureBook();
    const rerender = () => { if (here(sub) && window.__render) window.__render(); };
    const owner = bookId();
    if (!sub) {
    ...
    // The tell Start/Save fired: repaint when it answers, and read who was told.
    const t = toldState(sub);
    if (t && t.state === 'sending' && !t.watched) {
      t.watched = true;
      t.done.then(() => {
        const ph = hubPhase(VC.upcomingFor(sub) || []);
        const p = ph.phase === 'before' && ph.instance ? loadArming(ph.instance.instance_id, true) : Promise.resolve();
        return p.then(() => rerender());
      }, () => rerender());
    }
    ...
    on('#rhb-remind', async (b) => {
      ...
      HUB.note = { key, text: SAY[0], error: SAY[1] };
    ...
    on('#rhb-nudge', async (b) => {
      ...
      const key = `nudge:${id}`;
      b.disabled = true; b.setAttribute('aria-busy', 'true'); b.textContent = 'Sending…';
      const r = await remindMissing(id);
      const text = r.reason === 'ok' ? (r.sent ? `Nudged ${r.sent}. Each gets their own I’m Up.` : 'Everyone is up now.')
        : r.reason === 'rate_limited' ? 'They were nudged in the last 10 minutes. Try again after that.'
        : r.reason === 'not_authorized' ? 'Only the coaching staff can nudge.'
        : 'Couldn’t send. Check your connection and try again.';
      if (r.reason === 'ok' || r.reason === 'rate_limited') HUB.sentAt.set(key, Date.now());
      say(key, text, r.reason === 'failed' || r.reason === 'not_authorized');
