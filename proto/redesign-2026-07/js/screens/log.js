...
export default {
  ...
  render() {
    ...
    // Mirrors Home's nowCard rule: the hero wears the hue of the thing it asks for (green meal,
    // purple recovery, blue commitment, cyan weekly). Amber stays only when it is TRUE (window
    // closing, or late-but-savable); a decided miss is red.
    const od = n ? n.state === 'overdue' : false;
    const missed = od && n.color === 'red';
    const closing = n ? (!od && n.minsLeft != null && n.minsLeft <= 45) : false;
