...
export default {
  ...
  render() {
    ...
    <div class="lrow" id="note-row" style="border:1px solid var(--hairline);border-radius:15px;padding:13px 15px">
  ...
  async mount(root) {
    ...
    wireToggles(root);
    const row = root.querySelector('#note-row');
    if (row) row.addEventListener('click', () => {
      row.outerHTML = `<div class="composer" style="margin-top:0"><input placeholder="Note for ${S.coach.name}… (he sees it with the check-in)" autofocus /><div class="send" style="background:linear-gradient(150deg, var(--purple-bright), #7e22ce)">✓</div></div>`;
    });
  },
};
