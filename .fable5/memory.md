...
## Roadmap
- 2026-07-12: calibration gate PASSED (founder approved direction + green pill). Home pass 1 complete (3 shipped). Next: continue the coverage ledger (`.fable5/coverage.md`) — camera → meal → log → breakdown teardowns dispatched; ship top improvement per screen, one per cycle. Then: (a) first unlocked Practice HQ section — client health list is the highest-value candidate (real client rows already reachable via widened `fetchMyPractices`); (4) founder decision on the `practice_identity()` RPC proposal.
