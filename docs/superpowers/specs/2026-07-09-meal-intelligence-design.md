...
- **AI opening message (no extra AI call, no storage):** the AI Nutritionist's first message is **derived, not persisted** — a pure, unit-tested `openingMessage(...)` helper renders it from the meal's stored analysis (quality, note, foods, goal): the coaching `note`, a why-it-matters line for the user's goal, and one practical improvement when `quality < 75` (praise instead for high-quality meals). Both the athlete's thread and the coach's meal screen derive it identically from the same meal row, so every meal shows an immediate personalized response, it can never be forged, and it costs zero storage and zero AI calls. (Design correction from review: `meal_comments`' RLS deliberately forbids clients inserting `role='ai'` rows — 0046 reserves them for a service-role writer — so the client-persisted opening in the earlier draft was impossible; deriving is both feasible and stronger.)
- **Follow-up Q&A:** the composer ("Ask about this meal…") posts the athlete's message to the thread (`role='athlete'`, allowed by RLS), then invokes `meal-chat` (§6) with client-composed context: this meal (foods + confidence, macros, fiber, quality, timing), plan targets + goal + allergies, today's exec summary (met/total, score, next), the last 7 days of meals (name/macros/quality from the `meals` table, best-effort fetch), and the thread messages so far (≤20, truncated oldest-first). **The AI reply is persisted server-side by `meal-chat` itself** (service role, `role='ai'`, `author_id = athlete_id`) after verifying via the caller's JWT that the meal belongs to the caller — preserving 0046's unforgeability boundary. Answers are contextual to *their* plan — never generic nutrition advice.
...
- **Migrations (authored only, founder applies at go-live):** `meal_comments.role` already includes `'ai'` (0046) — no change needed. Add `kind text not null default 'message' check (kind in ('message','reaction'))` for reactions; existing insert policies already permit coach rows, so reactions need no policy change.
