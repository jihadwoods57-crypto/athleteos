...
| Native Sign in with Apple / Google does not complete on the reviewer's iPad (the server side is enabled; the client ids inside Supabase's provider config cannot be read from here) | 8% | Tap both on a real device once. Both must land on Home. | founder |
