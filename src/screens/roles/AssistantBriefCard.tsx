...
//   * AssistantKpiStrip  — the demoted metrics, one quiet line under the brief.
...
export function AssistantBriefCard({ brief, live }: { brief: AssistantBrief; live: boolean }) {
  ...
  return (
    <View style={{ marginTop: 20 }}>
      ...
      <Txt w="sb" size={16} color={c.text} style={{ marginTop: 10, lineHeight: 25, letterSpacing: -0.2 }}>
        {text}
      </Txt>
      {since ? (
...
/** The demoted metric strip: one quiet line of evidence under the brief. */
...
function TriageRow({ action, sent, onAct, onOpen }: {
  ...
  return (
    <View style={{ borderRadius: 16, padding: 14, backgroundColor: surface, marginTop: 8 }}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Open ${action.name}`} onPress={onOpen} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}>
        <Row style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <Txt w="eb" num size={17} color={accent}>{action.score}</Txt>
        </Row>
      </Pressable>
      {sent ? (
        <Row style={{ gap: 6, marginTop: 10, alignItems: 'center' }}>
        ...
        <Pressable
          ...
          onPress={onAct}
          style={({ pressed }) => ({ marginTop: 10, height: 40, borderRadius: 11, backgroundColor: accent, alignItems: 'center', justifyContent: 'center', opacity: pressed ? 0.85 : 1 })}
        >
...
export function TriageQueue({ brief, rosterMeta, viewAllCount, onViewAll }: {
  ...
  if (entries.length === 0) {
    return (
      <View style={{ marginTop: 14, borderRadius: 16, padding: 16, backgroundColor: c.successSurface }}>
    ...
    <View style={{ marginTop: 10 }}>
      <Row style={{ justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {viewAllCount && viewAllCount > brief.actions.length && onViewAll ? (
          <Pressable accessibilityRole="button" accessibilityLabel={`View all ${viewAllCount} who need attention`} hitSlop={6} onPress={() => { haptics.tap(); onViewAll(); }}>
...
export function AssistantUpgradeCard({ brief, noun }: { brief: AssistantBrief; noun: string }) {
  ...
  return (
    <Card variant="hero" style={{ marginTop: 14, borderRadius: 20 }}>
      ...
      <Txt w="sb" size={15.5} style={{ marginTop: 10, lineHeight: 23 }}>
        {k.total === 0
