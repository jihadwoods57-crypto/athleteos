...
test('weeklyBrief: completion delta computed from two weeks of rollup rows', () => {
  ...
  const b = weeklyBrief({ rollup, roster, todayISO: TODAY } as never);
...
test('athletesToWatch: a clean negative slope is a decliner; a 2-scored-day athlete is not', () => {
  ...
  const w = athletesToWatch({ rollup, roster, todayISO: TODAY } as never);
...
test('interventionOutcomes: locked under 14-day span even with 5 qualifying outcomes', () => {
  ...
  const r = interventionOutcomes({ outcomes, roster, todayISO: TODAY } as never);
...
test('determinism: same input twice -> deep-equal output', () => {
  ...
  expect(weeklyBrief({ rollup, roster, todayISO: TODAY } as never)).toEqual(weeklyBrief({ rollup, roster, todayISO: TODAY } as never));
  expect(athletesToWatch({ rollup, roster, todayISO: TODAY } as never)).toEqual(athletesToWatch({ rollup, roster, todayISO: TODAY } as never));
  expect(weekVsMonth({ rollup, todayISO: TODAY } as never)).toEqual(weekVsMonth({ rollup, todayISO: TODAY } as never));
  expect(interventionOutcomes({ outcomes, roster, todayISO: TODAY } as never)).toEqual(interventionOutcomes({ outcomes, roster, todayISO: TODAY } as never));
});
