// OnStandard — Command Center. Uses ONLY the publishable key + the founder's login JWT; every read
// is a platform-admin-gated RPC. All dynamic values go through textContent (never innerHTML), so no
// data path can inject markup. Decision logic lives in the pure, jest-tested attention.js — this
// file fetches, assembles the metrics bundle, renders, and drives actions/drill-downs/notifications.
...
const PROJECT_REF = 'ftwrvylzoyznhbzhgism';
...
const numN = (v) => { if (v == null) return null; const x = Number(v); return isFinite(x) ? x : null; };
const one = (rows) => (Array.isArray(rows) && rows.length ? rows[0] : {});
const usd4 = (v) => (v == null ? '—' : `$${num(v).toFixed(4)}`);
const usd2 = (v) => `$${num(v).toFixed(2)}`;
...
function card(title, nodes, headLink) {
...
function deltaOf(cur, prev, goodUp = true) {
...
function closeModal() { $('modal-root').textContent = ''; document.removeEventListener('keydown', escClose); }
...
async function loadAll(opts = {}) {
  if (!opts.silent) { $('err').textContent = ''; }
  try {
    const D = STATE.days;
    ...
    $('stamp').textContent = ''; $('stamp').appendChild(h('span', {}, ['updated ', h('b', { text: 'now' })]));
  } catch (e) {
    if (!opts.silent) $('err').textContent = 'Not authorized or unavailable: ' + e.message;
  }
}
...
function render(m, d) {
  ...
  $('foot').textContent = 'MRR is a placeholder estimate (sync rates with plans.ts). Evidence buttons copy a ready-to-run query and open Supabase Studio. Attention items you resolve/snooze are audited. A daily cron snapshot keeps the trend complete even when you don\'t visit.';
}
...
function renderPanels(m, d) {
  ...
  g.appendChild(card('Revenue', [
    h('div', { class: 'big num sig', text: usd2(num(d.rev.mrr_estimate_usd)) }),
    h('div', { class: 'cap', text: 'MRR · estimate' }),
...
function openAsk() {
  ...
  const filter = h('input', { type: 'text', placeholder: 'Ask about the business…', oninput: () => render() });
  function render() {
    list.textContent = '';
...
async function gate() {
  ...
  show($('login'), !signedIn); show($('app'), signedIn);
  if (signedIn) { await loadAll(); startPolling(); }
}
let pollTimer = null;
function startPolling() {
  if (pollTimer) return;
  pollTimer = setInterval(() => { if (document.visibilityState === 'visible') loadAll({ silent: true }); }, 5 * 60 * 1000);
}
...
$('signout').onclick = async () => { if (pollTimer) clearInterval(pollTimer), pollTimer = null; await sb.auth.signOut(); location.reload(); };
