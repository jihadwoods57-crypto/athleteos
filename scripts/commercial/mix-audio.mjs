...
//
// Usage: node scripts/commercial/mix-audio.mjs <silent.mp4> <out.mp4>
...
if (!src || !out) { console.error('usage: node mix-audio.mjs <silent.mp4> <out.mp4>'); process.exit(1); }

const musicDir = join(here, 'music');
const licensed = existsSync(musicDir)
