...
// `--check` DOES NOT WRITE. It asks the one question the other zip gate cannot: is the artifact we
// actually ship the artifact this source builds? verify:zip reads assets/proto.zip and proves that
// zip parses and resolves its imports, which a zip built from LAST week's proto also does. So a
// proto edit that never got rebuilt passed every gate green and then shipped nothing: the OTA
// carries the zip, not proto/redesign-2026-07/. That has cost this repo a release more than once.
// Safe as a gate only because the build above is byte-deterministic (fixed mtime, LF-normalised
// text), so "differs" always means stale and never means "built on a different machine".
if (process.argv.includes('--check')) {
  const committed = (() => { try { return new Uint8Array(readFileSync(OUT_ZIP)); } catch { return null; } })();
  const same = committed && committed.length === zipped.length && committed.every((b, i) => b === zipped[i]);
  const verSame = (() => { try { return readFileSync(OUT_VER, 'utf8') === VER_BODY; } catch { return false; } })();
  if (same && verSame) {
    console.log(`proto.zip: up to date with proto/redesign-2026-07 (version ${hash})`);
    process.exit(0);
  }
  console.error('✗ assets/proto.zip is STALE: it does not match proto/redesign-2026-07.');
  if (!committed) console.error('  assets/proto.zip is missing entirely.');
  else if (!same) console.error(`  shipped zip is ${committed.length} bytes, this source builds ${zipped.length}.`);
  if (!verSame) console.error(`  src/proto/protoVersion.ts does not carry ${hash}.`);
  console.error('  The OTA ships the ZIP, not the proto directory. Run: node scripts/build-proto-zip.mjs');
  process.exit(1);
}
...
writeFileSync(OUT_VER, VER_BODY);
